...
var gx = {
	...
	evt: {
			...
			if (!gx.lang.emptyObject(gridId))
			{
				if (gx.lang.emptyObject(rowId))
				{
					...
					if (gridObj)
					{
						...
						if (gridObj.grid.hasPagingButtons())
							gridObj.setPagingVars('0', '0');
					}
				}
		...
		_init: function() {
			...
			if (gx.util.browser.isIE() && (gx.util.browser.isCompatMode() || gx.util.browser.ieVersion() <=8 && !gx.util.browser.isWinCE()  ) ){					
...
gx.popup = {
	...
	ext: {
		...
		close: function(popupObj) {
			...
			if (gx.util.browser.isFirefox() && iFrame)
		...
		isPDF_IE: function(iFrame){
			...
			if (gx.util.browser.isIE())
			{				
				var src = (iFrame.src.indexOf('?')<0)?iFrame.src:iFrame.src.substring(0,iFrame.src.indexOf('?'));				
...
gx.grid.impl = function(id) {
	...
	this.getColumnByHtmlName = function(htmlName) {
		...
		if (typeof(column) == 'undefined' && typeof(this.CmpContext) != 'undefined' && this.CmpContext.length < htmlName.length){
			varCxt = htmlName.substring(0,this.CmpContext.length);
			if (varCxt == this.CmpContext){
				htmlName = htmlName.substring(this.CmpContext.length , htmlName.length);
				column = this.columnsHtmlName[htmlName];			
			}
		}
		return column;
